    ...
    <div class="py-6 max-w-7xl mx-auto">
        <div class="user-form-card">
            <div class="form-header">
                <a href="{{ route('admin.departments.index') }}" class="modal-close">&times;</a>
                <div class="form-title">Birim Düzenle: {{ $department->name }}</div>
            </div>
            <div class="form-body">
                <form id="department-edit-form" method="POST" action="{{ route('admin.departments.update', $department->id) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Birim Adı</label>
                        <input type="text" name="name" value="{{ old('name', $department->name) }}" required class="w-full">
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>
                    <div class="flex justify-end form-footer">
                         <button type="submit" form="department-edit-form" class="submit-animated" id="submit-button">
                            <span>Güncelle</span>
