    ...
    <div class="py-6 max-w-7xl mx-auto">
        <div class="user-form-card">
            <div class="form-header">
                ...
                <div class="form-title">Yeni Birim Ekle</div>
            </div>
            <div class="form-body">
                <form id="department-form" method="POST" action="{{ route('admin.departments.store') }}" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Birim Adı</label>
                        <input type="text" name="name" value="{{ old('name') }}" required class="w-full">
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>
                    <div class="flex justify-end form-footer">
                         <button type="submit" form="department-form" class="submit-animated" id="submit-button">
