<x-app-layout>
    <style>
        ...
        /* Sadece bu sayfa: .sec-page scope */
        .sec-page .card{
            border-radius: 22px;
            background: #fff;
        ...
        /* Tablo kartı: dışta radius kalsın, içteki scroll düz olsun */
        .sec-page .table-card{
        padding: 0;              /* iç boşluk kaldır */
        border-radius: 22px;     /* dış kartın köşeleri */
        }

        /* İç scroll alanı: köşeleri düz, sadece yatay scroll */
        .sec-page .table-scroll{
        overflow-x: auto;
        overflow-y: hidden;
        border-radius: 0;        /* alt kısım düz dursun */
        }

        /* İsteğe bağlı: scrollbar'ı incelt ve arka planı şeffaf yap */
        .sec-page .table-scroll::-webkit-scrollbar{ height: 6px; }
        .sec-page .table-scroll::-webkit-scrollbar-track{ background: transparent; }
        .sec-page .table-scroll::-webkit-scrollbar-thumb{
        background: #e5e7eb; border-radius: 999px;
        }

        /* --- Mevcut stillerin: toggle, buton, inputlar --- */
        .toggle-switch{width:80px;height:36px;background:#cbd5e1;border-radius:999px;border:none;cursor:pointer;position:relative;transition:background .4s ease;box-shadow:inset 0 2px 4px rgba(0,0,0,.1)}
        .toggle-switch.active{background:linear-gradient(90deg,#22c55e,#16a34a)}
        .toggle-switch .circle{position:absolute;top:3px;left:4px;width:30px;height:30px;background:#fff;border-radius:50%;transition:left .3s ease;box-shadow:0 2px 6px rgba(0,0,0,.2)}
        .toggle-switch.active .circle{left:46px}
        .toggle-switch .label{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:12px;color:#fff;font-weight:600;text-shadow:0 0 3px rgba(0,0,0,.7);pointer-events:none;z-index:1}
        ...
        a.text-blue-600{display:inline-block;padding:8px 18px;background:linear-gradient(135deg,rgba(59,130,246,.15),rgba(59,130,246,.05));border:1px solid rgba(59,130,246,.3);border-radius:12px;color:#1d4ed8;font-weight:600;font-size:14px;text-align:center;box-shadow:0 4px 10px rgba(59,130,246,.1);transition:.25s;backdrop-filter:blur(6px)}
            ...
            if (toggleFormButton && kayıtFormu) {
                toggleFormButton.addEventListener('click', function () {
                    ...
                    if (!kayıtFormu.classList.contains('hidden')) {
                        ...
                        // Burada default rol ayarlayabilirsin
                ...
                <div class="card-body">
                    <form method="POST" action="{{ isset($editUser) ? route('admin.users.update', $editUser->id) : route('admin.users.store') }}">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium">{{ isset($editUser) ? 'Yeni Şifre' : 'Şifre' }}</label>
                    ...
                    <tbody>
                        @foreach($users as $index => $user)
                            <tr class="border-t {{ !$user->is_active ? 'passive-row' : '' }}">
                                ...
                                <td class="px-4 py-2">
                                    @if(auth()->user()->role === 'super_admin')
                                        <a href="{{ route('admin.users.index', ['edit' => $user->id]) }}" class="text-blue-600">Düzenle</a>
                                    @else
